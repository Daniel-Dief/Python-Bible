...
def getAbbrev(book : str):
    """
    Returns the abbreviated book name.\n
...
def threadBookChapter(cmb_book : Combobox, cmb_chapter : Combobox):
    """
    ...
    while True:
        ...
        try:
            if cmb_book.get() != last:
                last = cmb_book.get()
                chapters = getBook(getAbbrev(cmb_book.get()))
                array_aux = []
                for i in range(chapters['chapters']+1):
                    array_aux.append(i)
...
def threadChapterVerse(cmb_book : Combobox, cmb_chapter : Combobox, cmb_verse : Combobox):
    """
    Prepare a function to use per a thread in checkBook().\n
...
def checkSearch(cmb_book : Combobox, cmb_chapter : Combobox, cmb_verse : Combobox):
    """
    ...
    thread_book_chapter = Thread(target=threadBookChapter, args=(cmb_book, cmb_chapter))
    thread_chapter_verse = Thread(target=threadChapterVerse, args=(cmb_book, cmb_chapter, cmb_verse))
